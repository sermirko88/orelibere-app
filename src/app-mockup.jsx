...
function GuidaScreen({ onBack, redditoTipo }) {
  ...
  const SEZIONI = [
    ...
    },
    {
      ...
      esempio: "Serve per capire quanto ti costa davvero, in ore, comprare qualcosa prima di farlo. Scrivi il prezzo, scegli come lo pagheresti (subito, a rate PayPal, o a rate con finanziamento), e l'app confronta le ore di lavoro che ti costa ogni opzione — utile soprattutto per capire quanto ti costano gli interessi.",
    },
    {
      id: "budget",
      titolo: "Budget — i tuoi obiettivi",
      esempio: "Serve per mettere via soldi per qualcosa di preciso — un viaggio, un acquisto, un fondo di sicurezza. Tocca \"+\", scrivi un nome, quanto ti serve ed entro quando (se vuoi una scadenza). L'app ti dice quanto mettere da parte ogni mese, e ti avvisa se non è realistico con quello che guadagni.",
    },
    {
      id: "chiusura",
      titolo: "Chiusura",
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", paddingBottom: 32 }}>
      ...
      <div style={{ padding: "0 20px", display: "flex", flexDirection: "column", gap: 8 }}>
        {SEZIONI.map((s) => (
          <div key={s.id} style={{ backgroundColor: C.panel, border: `1px solid ${C.panelBorder}`, borderRadius: 8, overflow: "hidden" }}>
            <button
              onClick={() => toggle(s.id)}
              style={{ width: "100%", padding: "13px 14px", background: "none", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between" }}
            >
              <span style={{ fontSize: 13.5, fontWeight: 700, color: C.paper }}>{s.titolo}</span>
              <ChevronDown size={16} color={C.textFaint} style={{ transform: open === s.id ? "rotate(180deg)" : "none", transition: "transform 0.15s" }} />
            </button>
            {open === s.id && (
