...
function OnboardingIncome({ data, setData, onNext }) {
  ...
  const hourlyFromRate = data.tariffaOrariaLorda ? Number(data.tariffaOrariaLorda) * ((Number(data.percentualeNetta) || 100) / 100) : null;
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "24px 20px 32px 20px", overflowY: "auto" }}>
      ...
      {isVariabile && data.usaOraria ? (
        <>
          <div style={{ marginBottom: 6 }}><FieldLabel>Tariffa oraria che fatturi (lorda)</FieldLabel></div>
          <div style={{ marginBottom: 16 }}>
            <TextInput type="number" value={data.tariffaOrariaLorda} placeholder="35" prefix="€" suffix="/h" big
          ...
          <div style={{ marginBottom: 6 }}><FieldLabel>Ore fatturabili a settimana in media</FieldLabel></div>
          <div style={{ marginBottom: 16 }}>
          ...
          <p style={{ fontSize: 11.5, color: C.textFainter, lineHeight: 1.5, marginTop: -10, marginBottom: 16 }}>
            Serve solo a stimare il tuo reddito mensile medio, per capire quanto margine hai davvero per gli obiettivi.
          </p>
        </>
      ) : (
        <>
          <div style={{ marginBottom: 6 }}><FieldLabel>{isVariabile ? "Reddito netto medio mensile" : "Stipendio netto mensile"}</FieldLabel></div>
          <div style={{ marginBottom: 8 }}>
          ...
          {isVariabile && (
            <p style={{ fontSize: 11.5, color: C.textFainter, lineHeight: 1.5, marginBottom: 16 }}>
              Meglio una media sugli ultimi 12 mesi (reddito netto totale ÷ 12) che l'ultimo mese: così pesi anche i periodi scarsi, non solo quelli buoni.
            </p>
          )}

          <div style={{ marginBottom: 6, marginTop: isVariabile ? 0 : 0 }}><FieldLabel>Ore lavorate a settimana{isVariabile ? " in media" : ""}</FieldLabel></div>
          <div style={{ marginBottom: 16 }}>
...
function OnboardingGoal({ data, setData, onNext, onBack }) {
  ...
  const hourlyForIncome = data.redditoTipo === "variabile" && data.usaOraria
    ? (Number(data.tariffaOrariaLorda) || 0) * ((Number(data.percentualeNetta) || 100) / 100)
    : null;
  const monthlyIncome = hourlyForIncome
...
const TUTORIAL_STEPS = [
  ...
  { tab: "diario", targetId: "tut-tabbar", radius: 12, text: "Da questa barra passi tra le sezioni dell'app. Diario, Simulatore e Budget ci sono sempre; le altre due compaiono quando servono." },
  { tab: "sim", targetId: "tut-tab-sim", radius: 16, text: "Il Simulatore ti mostra il costo reale di un acquisto prima di farlo: cash, a rate senza interessi, o a rate finanziate — confronta le ore che ti costa ciascuna opzione." },
  { tab: "goal", targetId: "tut-tab-goal", radius: 16, text: "Questo è Budget: qui vedi tutti i tuoi obiettivi di risparmio con l'avanzamento. Tocca \"+\" in alto per aggiungerne uno nuovo in qualsiasi momento." },
  { tab: "settings", targetId: "tut-bank-connect", radius: 10, text: "Tocca qui per collegare banca, Revolut o PayPal. Una volta collegati, l'app ti avvisa ogni volta che spendi qualcosa, pronta da categorizzare in un tocco: è la card \"Rendiconto\" che compare in basso." },
  { tab: "diario", targetId: "tut-tab-closure", radius: 16, text: "Ultima cosa: quando hai messo via qualcosa alla fine della settimana o del mese, comparirà qui in basso anche \"Chiusura\" — da lì decidi tu su quale obiettivo farlo atterrare." },
];
...
function CalendarioScreen({ calendario, setCalendario, hourlyEstimate, progetti, setProgetti }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", paddingBottom: 96 }}>
      <ScreenHeader
        eyebrow="Turni, entrate e uscite"
        title="Calendario"
        right={
          <button
              ...
              <div key={e.id} style={{ display: "flex", alignItems: "center", gap: 10, backgroundColor: C.bg, border: `1px solid ${C.panelBorder}`, borderRadius: 6, padding: "10px 12px", marginBottom: 8 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 12.5, color: C.paper, fontWeight: 600 }}>
                    {e.tipo === "turno" ? `Turno · ${e.ore}h` : e.tipo === "entrata" ? `Entrata · ${Number(e.importo).toFixed(0)}€` : `Uscita · ${Number(e.importo).toFixed(0)}€`}
...
function SettingsScreen({ data, setData, onBack, onFullOnboarding, onOpenTransactions, onChangeUser, onOpenRegime }) {
...
function MainApp({ currentUser, onChangeUser }) {
  ...
  const derivedMonthlyIncome = data.redditoTipo === "variabile" && data.usaOraria
    ...
    <div style={outerStyle}>
      <div style={frameStyle} ref={frameRef}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
          ...
          {tab === "settings" && <SettingsScreen data={data} setData={setData} onBack={() => setTab("diario")} onFullOnboarding={() => { setStep(0); setOnboarded(false); }} onOpenTransactions={() => setTab("transactions")} onChangeUser={onChangeUser} onOpenRegime={() => setTab("regime")} />}
        ...
        {(tab === "diario" || tab === "goal" || tab === "sim" || tab === "closure" || tab === "transactions" || tab === "calendario") && (
          <div id="tut-tabbar" style={{ borderTop: `1px solid ${C.panelBorder}`, backgroundColor: C.bg, display: "flex", alignItems: "center", justifyContent: "space-around", padding: "12px 16px" }}>
            ...
            {data.redditoTipo === "variabile" && (
              <button onClick={() => setTab("calendario")} style={{ background: "none", border: "none", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, cursor: "pointer" }}>
