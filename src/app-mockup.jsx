...
function WelcomeScreen({ onStart }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "32px 24px 28px 24px", overflowY: "auto" }}>
      ...
      `}</style>
      <div style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.25em", color: C.brass, fontFamily: SANS_FONT, fontWeight: 700, marginBottom: 18, animation: "wsFadeUp 0.6s ease 0ms both" }}>OreLibere</div>

      <h1 style={{ fontSize: 26, fontWeight: 800, color: C.paper, fontFamily: DISPLAY_FONT, margin: "0 0 6px 0", lineHeight: 1.2, animation: "wsFadeUp 0.6s ease 120ms both" }}>
        Non hai mai pagato il prezzo giusto.
      </h1>
      <p style={{ fontSize: 14, color: C.textFainter, fontFamily: SERIF_FONT, fontStyle: "italic", fontWeight: 500, margin: "0 0 24px 0", animation: "wsFadeUp 0.6s ease 280ms both" }}>
        Perché hai sempre guardato il numero sbagliato.
      </p>

      <p style={{ ...P, animation: "wsFadeUp 0.6s ease 440ms both" }}>
...
const TUTORIAL_STEPS = [
  ...
  { tab: "diario", targetId: "tut-tabbar", radius: 12, text: "Da questa barra passi tra le sezioni dell'app. Diario, Budget e Simulatore ci sono sempre; le altre due compaiono quando servono." },
  { tab: "goal", targetId: null, text: "Questo è Budget: qui vedi tutti i tuoi obiettivi di risparmio con l'avanzamento. Tocca \"+\" in alto per aggiungerne uno nuovo in qualsiasi momento." },
  { tab: "sim", targetId: null, text: "Il Simulatore ti mostra il costo reale di un acquisto prima di farlo: cash, a rate senza interessi, o a rate finanziate — confronta le ore che ti costa ciascuna opzione." },
  { tab: "settings", targetId: null, text: "Da qui colleghi banca, Revolut o PayPal. Una volta collegati, l'app ti avvisa ogni volta che spendi qualcosa, pronta da categorizzare in un tocco: è la card \"Rendiconto\" che compare in basso." },
  { tab: "diario", targetId: null, text: "Ultima cosa: quando hai messo via qualcosa alla fine della settimana o del mese, comparirà qui in basso anche \"Chiusura\" — da lì decidi tu su quale obiettivo farlo atterrare." },
];
...
function TutorialOverlay({ step, frameRef, onNext, onFinish }) {
...
function DiarioScreen({ profile, todayEntries, onOpenAdd, onOpenSettings, onOpenReport, onOpenGoal, onSimulateBankTx }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", paddingBottom: 96 }}>
      ...
      <div style={{ padding: "0 20px" }}>
        <PunchTicket id="tut-gauge" style={{ borderRadius: 4, padding: "24px 16px", border: `1px solid #E3DCC8` }}>
  ...
  return (
    <div style={outerStyle}>
      <div style={frameStyle} ref={frameRef}>
        {!tutorialDone && <TutorialOverlay step={tutorialStep} frameRef={frameRef} onNext={advanceTutorial} onFinish={skipTutorial} />}
        {syncError && (
          <div style={{ position: "absolute", top: 8, left: 8, right: 8, zIndex: 90, backgroundColor: C.rust, color: "#fff", borderRadius: 8, padding: "10px 12px", fontSize: 11.5, lineHeight: 1.4, display: "flex", alignItems: "flex-start", gap: 8 }}>
            <span style={{ flex: 1 }}>⚠ Sincronizzazione: {syncError}</span>
            <button onClick={() => setSyncError(null)} style={{ background: "none", border: "none", color: "#fff", cursor: "pointer", fontWeight: 700, padding: 0 }}>✕</button>
