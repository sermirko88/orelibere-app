...
// Mini-tour mostrato subito dopo lo sblocco di Premium: porta la persona a vedere,
// una per una, le funzioni che ha appena guadagnato — non l'intero giro di onboarding,
// solo le novità.
function buildPremiumUnlockSteps(isVariabile) {
  const steps = [
    { tab: "calendario", targetId: "tut-tab-calendario", radius: 16, text: "Sei Premium! Il Calendario ora è tuo: registra turni, entrate e uscite giorno per giorno, e l'app calcola la tua vera tariffa oraria al posto della stima." },
    { tab: "sim", targetId: "tut-tab-sim", radius: 16, text: "Nel Simulatore hai sbloccato anche il Finanziamento: confronta il costo delle rate con gli interessi, sempre convertiti in ore." },
    { tab: "goal", targetId: "tut-tab-goal", radius: 16, text: "In Obiettivi ora puoi tenerne quanti vuoi in parallelo — viaggio, fondo emergenza, il prossimo acquisto, tutti insieme." },
    { tab: "diario", targetId: "tut-tab-closure", radius: 16, text: "E quando il periodo si chiude, trovi qui anche \"Chiusura\": da lì decidi su quale obiettivo far atterrare il risparmio." },
    { tab: "settings", targetId: "tut-bank-connect", radius: 10, text: "Da Impostazioni puoi anche collegare i tuoi conti e importare spese da file CSV o PDF, invece di inserirle a mano." },
  ];
  return steps;
}

// Mini-tour mostrato subito dopo lo sblocco di Elite.
function buildEliteUnlockSteps(isVariabile) {
  const steps = [
    { tab: "settings", targetId: null, text: "Sei Elite! Da Impostazioni trovi ora \"Regime fiscale\": stima quanto ti resta in tasca dopo tasse e contributi, forfettario o ordinario che sia." },
  ];
  if (isVariabile) {
    steps.push({ tab: "calendario", targetId: null, text: "Dal Calendario, tocca \"Progetti\" per scoprire la tua tariffa oraria reale su ogni lavoro che accetti — prezzo diviso ore." });
  }
  return steps;
}
...
function MainApp({ currentUser, onChangeUser }) {
  ...
  const [tutorialMode, setTutorialMode] = useState("onboarding"); // "onboarding" | "premium" | "elite" — quale giro di tutorial è in corso
  ...
  const tutorialSteps = tutorialMode === "premium"
    ? buildPremiumUnlockSteps(data.redditoTipo === "variabile")
    : tutorialMode === "elite"
    ? buildEliteUnlockSteps(data.redditoTipo === "variabile")
    : buildTutorialSteps(data.redditoTipo === "variabile");
  ...
  const finishOnboarding = () => {
    ...
    if (reOnboarding) {
      ...
      setTutorialStep(0);
      setTab(buildTutorialSteps(data.redditoTipo === "variabile")[0].tab);
    }
  };
  ...
  const advanceTutorial = () => {
    ...
    if (nextStep >= tutorialSteps.length) {
      ...
      setTutorialMode("onboarding"); // torna al giro completo di default per i prossimi richiami
      setTab("diario");
    } else {
  ...
  // mostra sempre il giro completo, non un mini-tour di sblocco
  const replayTutorial = () => {
    const steps = buildTutorialSteps(data.redditoTipo === "variabile");
    setTutorialMode("onboarding");
    setTutorialStep(0);
  ...
  // Subito dopo (una volta chiuso il foglio di conferma) parte un mini-tour guidato
  // solo sulle funzioni appena sbloccate, non l'intero onboarding.
  const handleUnlockTier = (tier, priceEuro, label) => {
    ...
    setTimeout(() => {
      const mode = tier === "elite" ? "elite" : "premium";
      const steps = mode === "elite" ? buildEliteUnlockSteps(data.redditoTipo === "variabile") : buildPremiumUnlockSteps(data.redditoTipo === "variabile");
      setTutorialMode(mode);
      setTutorialStep(0);
      setTutorialDone(false);
      setTab(steps[0].tab);
    }, 1500); // stesso tempo del messaggio "sbloccato" nel foglio del paywall, così il tour parte subito dopo che si chiude
  };
