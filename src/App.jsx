...
function OneTapCategorizeSheet({ tx, hourly, onClose, onConfirm }) {
  ...
  const pick = (cat) => {
    setDone(true);
    playExpenseSound();
    onConfirm({ cat: cat.label, icon: cat.icon, euro: tx.euro, time: "adesso" });
...
function AddSheet({ hourly, onClose, onAdd }) {
  ...
  const handleAmount = (val) => {
    ...
    setStep("done");
    playExpenseSound();
    onAdd({ cat: category.label, icon: category.icon, euro: val, time: "adesso" });
...
function SettingsScreen({ data, setData, onBack, onFullOnboarding, onOpenTransactions }) {
...
export default function App() {
  ...
  const [entries, setEntries] = useState([
    { cat: "Colazione", icon: Coffee, euro: 2.5, time: "08:12" },
    { cat: "Pranzo", icon: UtensilsCrossed, euro: 9, time: "13:02" },
  ]);
  ...
  if (!onboarded) {
    ...
    return (
      <div style={outerStyle}>
        <div style={frameStyle}>{steps[step]}</div>
      </div>
    );
  }
    ...
    <div style={outerStyle}>
      <div style={frameStyle}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
          ...
          {tab === "settings" && <SettingsScreen data={data} setData={setData} onBack={() => setTab("diario")} onFullOnboarding={() => setOnboarded(false)} onOpenTransactions={() => setTab("transactions")} />}
          {tab === "transactions" && (
            <TransactionsScreen
              hourly={hourlyRate}
