...
import {
  ...
  Cigarette, Wifi, ArrowRight, Settings2, CreditCard, ShoppingBag, Gift, HeartPulse, BarChart3, ChevronLeft, ChevronRight, Lightbulb, PiggyBank, Landmark, Bell, Info, HandCoins, ExternalLink, TriangleAlert, Calendar, TrendingUp, Clock
} from "lucide-react";
...
// Cerca righe nel formato "data ... importo" — funziona bene quando ogni movimento
// sta su una riga sola (il caso più comune); se una banca spezza il movimento su più
// righe, quella riga semplicemente non viene riconosciuta.
function parseTransactionLines(text) {
  ...
  for (const line of lines) {
    ...
    // se ci sono 2+ numeri sulla riga, l'ultimo è quasi sempre il saldo progressivo
    ...
    desc = desc.replace(/\b(dare|avere)\b/i, "").trim();
...
function buildTutorialSteps(isVariabile) {
  const steps = [
    ...
    { tab: "diario", targetId: "tut-tabbar", radius: 12, text: isVariabile
      ...
      : "Da questa barra passi tra le sezioni dell'app. Diario, Simulatore e Budget ci sono sempre; le altre due compaiono quando servono." },
  ];

  if (isVariabile) {
    steps.push({ tab: "calendario", targetId: "tut-tab-calendario", radius: 16, text: "Il Calendario è pensato per te: segna turni, entrate e uscite, pianificati o già confermati. Da qui trovi anche \"Progetti\", per capire quanto rendono davvero i lavori che accetti." });
  }

  steps.push({ tab: "sim", targetId: "tut-tab-sim", radius: 16, text: "Il Simulatore ti mostra il costo reale di un acquisto prima di farlo: cash, a rate senza interessi, o a rate finanziate — confronta le ore che ti costa ciascuna opzione." });
  steps.push({ tab: "goal", targetId: "tut-tab-goal", radius: 16, text: "Questo è Budget: qui vedi tutti i tuoi obiettivi di risparmio con l'avanzamento. Tocca \"+\" in alto per aggiungerne uno nuovo in qualsiasi momento." });
  ...
  steps.push({ tab: "settings", targetId: "tut-import-csv", radius: 8, text: "Se hai già un estratto conto scaricato, puoi importarlo qui in un colpo solo: l'app legge i movimenti e te li mostra in ore, non solo in euro, divisi giorno per giorno nel Calendario." });
        ...
        {status !== "parsed" && !result && (
          <div style={{ display: "flex", alignItems: "flex-start", gap: 8, backgroundColor: "rgba(255,107,74,0.1)", border: `1px solid ${C.brass}`, borderRadius: 8, padding: "10px 12px", marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 11.5, color: C.paper, lineHeight: 1.5 }}>
              Funziona solo con PDF con testo vero (non scansioni/foto), e legge meglio i formati con una transazione per riga. Se disponibile, il CSV resta più affidabile.
            </span>
          </div>
        )}
          ...
          <>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 8, backgroundColor: "rgba(255,107,74,0.14)", border: `1px solid ${C.brass}`, borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11.5, color: C.textFaint, lineHeight: 1.45 }}>Leggere un PDF è meno affidabile del CSV: importi o date potrebbero essere letti male. Dai un'occhiata all'anteprima sotto prima di importare.</div>
              </div>
            </div>
...
function ImportEstrattoContoScreen({ calendario, setCalendario, onBack }) {
  ...
  const handleFile = async (file) => {
    ...
    try {
      ...
      if (isExcel) {
        ...
        header = allRows[0].map((c) => String(c));
        dataRows = allRows.slice(1).filter((r) => r.some((c) => String(c).trim())).map((r) => r.map((c) => String(c)));
      } else {
        const text = await file.text();
        const { rows } = parseCSV(text);
        if (rows.length < 2) { setError("Il file sembra vuoto o non è un CSV valido."); return; }
        header = rows[0];
        dataRows = rows.slice(1).filter((r) => r.length > 1 || (r[0] && r[0].trim()));
      }
      setParsed({ header, rows: dataRows });

      // Primo tentativo: nome della colonna. Se non trova nulla, guarda i dati veri.
      const findCol = (patterns) => header.findIndex((h) => patterns.some((p) => (h || "").toLowerCase().includes(p)));
      let cData = findCol(["data", "date"]);
      let cDesc = findCol(["descrizione", "causale", "description", "dettagli", "beneficiario"]);
  ...
  const doImport = () => {
    ...
    parsed.rows.forEach((row) => {
      const d = parseFlexibleDate(row[colData]);
      const importoRaw = parseItalianNumber(row[colImporto]);
      if (!d || !importoRaw) return;
      const k = dateKey(d);
      const tipo = importoRaw < 0 ? "uscita" : "entrata";
      const entry = { id: Date.now() + Math.random(), tipo, stato: "consuntivo", importo: Math.abs(importoRaw), descrizione: colDesc >= 0 ? (row[colDesc] || "") : "" };
      newCal[k] = [...(newCal[k] || []), entry];
      if (tipo === "entrata") entrate += entry.importo; else uscite += entry.importo;
      count++;
    });
                ...
                <table style={{ borderCollapse: "collapse", fontSize: 11, width: "100%" }}>
                  ...
                  <tbody>
                    {parsed.rows.slice(0, 4).map((row, ri) => (
                      <tr key={ri}>
                        <td style={{ padding: "6px 8px", color: C.textDim, whiteSpace: "nowrap" }}>{row[colData]}</td>
                        <td style={{ padding: "6px 8px", color: C.textDim, whiteSpace: "nowrap", maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis" }}>{colDesc >= 0 ? row[colDesc] : "—"}</td>
                        <td style={{ padding: "6px 8px", color: C.textDim, whiteSpace: "nowrap" }}>{row[colImporto]}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div style={{ overflowX: "auto", marginTop: 6, border: `1px solid ${C.panelBorder}`, borderRadius: 6 }}>
                <table style={{ borderCollapse: "collapse", fontSize: 11, width: "100%" }}>
                  ...
                  <tbody>
                    {parsed.rows.slice(0, 4).map((row, ri) => (
                      <tr key={ri}>
                        {row.map((c, ci) => <td key={ci} style={{ padding: "6px 8px", color: C.textDim, whiteSpace: "nowrap" }}>{c}</td>)}
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function CalendarioScreen({ calendario, setCalendario, hourlyEstimate, progetti, setProgetti }) {
  const [viewDate, setViewDate] = useState(new Date());
  const [selectedDay, setSelectedDay] = useState(null);
  const [showAdd, setShowAdd] = useState(false);
  const [showProgetti, setShowProgetti] = useState(false);
  const [form, setForm] = useState({ tipo: "turno", ore: "", importo: "", descrizione: "" });
  ...
  const addEntry = () => {
    ...
    setForm({ tipo: "turno", ore: "", importo: "", descrizione: "" });
    setShowAdd(false);
  };
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", paddingBottom: 96 }}>
      <ScreenHeader
        eyebrow="Entrate, uscite e turni"
        title="Calendario"
        right={
          <button
            ...
            {showAdd ? (
              <div style={{ marginTop: 8 }}>
                <div style={{ display: "flex", gap: 6, backgroundColor: C.bg, border: `1px solid ${C.panelBorder}`, borderRadius: 999, padding: 4, marginBottom: 14 }}>
                  {[["turno", "Turno"], ["entrata", "Entrata"], ["uscita", "Uscita"]].map(([id, label]) => (
                    <button
                      key={id}
...
function SettingsScreen({ data, setData, onBack, onFullOnboarding, onOpenTransactions, onChangeUser, onOpenRegime, onOpenImport, onOpenImportPDF }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", paddingBottom: 32 }}>
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        >
          <ArrowRight size={14} color={C.brass} style={{ transform: "rotate(-90deg)" }} /> Importa estratto conto (CSV/Excel)
        </button>
        ...
        <button
          ...
          style={{
            width: "100%", padding: "12px 0", borderRadius: 4, border: `1px solid ${C.panelBorder}`, background: "none",
            color: C.textDim, fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginBottom: 24,
          }}
        >
          <ArrowRight size={14} color={C.brass} style={{ transform: "rotate(-90deg)" }} /> Importa estratto conto (PDF)
        </button>
...
function MainApp({ currentUser, onChangeUser }) {
  ...
  const realRateInfo = data.redditoTipo === "variabile" ? computeRealRate(data.calendario) : { ready: false };
    ...
    <div style={outerStyle}>
      <div style={frameStyle} ref={frameRef}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
          ...
          {tab === "settings" && <SettingsScreen data={data} setData={setData} onBack={() => setTab("diario")} onFullOnboarding={() => { setStep(0); setOnboarded(false); }} onOpenTransactions={() => setTab("transactions")} onChangeUser={onChangeUser} onOpenRegime={() => setTab("regime")} onOpenImport={() => setTab("importcsv")} onOpenImportPDF={() => setTab("importpdf")} />}
        ...
        {(tab === "diario" || tab === "goal" || tab === "sim" || tab === "closure" || tab === "transactions" || tab === "calendario") && (
          <div id="tut-tabbar" style={{ borderTop: `1px solid ${C.panelBorder}`, backgroundColor: C.bg, display: "flex", alignItems: "center", justifyContent: "space-around", padding: "12px 16px" }}>
            ...
            {(data.redditoTipo === "variabile" || Object.keys(data.calendario || {}).length > 0) && (
