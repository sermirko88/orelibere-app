  ...
  const handleFile = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        setColData(findCol(["data", "date"]));
        setColDesc(findCol(["descrizione", "causale", "description", "dettagli", "beneficiario"]));
        setColImporto(findCol(["importo", "amount", "valore", "dare", "avere"]));
        setResult(null);
      } catch (err) {
        setError("Non sono riuscito a leggere il file. Assicurati che sia un CSV.");
      }
    };
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 70, backgroundColor: C.bg, display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "12px 20px 32px 20px" }}>
        <p style={{ fontSize: 12, color: C.textFaint, lineHeight: 1.5, marginBottom: 16 }}>
          Carica il file CSV dei movimenti (lo trovi di solito nella sezione "estratto conto" o "movimenti" della tua app bancaria, come esportazione). Ogni banca lo formatta un po' diversamente: prima di importare potrai controllare che le colonne siano lette bene.
        </p>
        ...
        {!parsed && (
          <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8, border: `1px dashed ${C.panelBorder}`, borderRadius: 8, padding: "32px 16px", cursor: "pointer", backgroundColor: C.panel }}>
            ...
            <span style={{ fontSize: 13, color: C.paper, fontWeight: 600 }}>Scegli il file CSV</span>
            <span style={{ fontSize: 11, color: C.textFaint, textAlign: "center" }}>Formato .csv esportato dalla tua banca</span>
        ...
        {parsed && !result && (
          <>
            ...
            <button onClick={() => { setParsed(null); setError(null); }} style={{ width: "100%", padding: "10px 0", background: "none", border: "none", color: C.textDim, fontSize: 12, marginTop: 10, cursor: "pointer" }}>Scegli un altro file</button>
          </>
        )}
...
function SettingsScreen({ data, setData, onBack, onFullOnboarding, onOpenTransactions, onChangeUser, onOpenRegime, onOpenImport }) {
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", paddingBottom: 32 }}>
      ...
      <div style={{ padding: "0 20px" }}>
        ...
        >
          <ArrowRight size={14} color={C.brass} style={{ transform: "rotate(-90deg)" }} /> Importa estratto conto (CSV)
        </button>
    ...
    <div style={outerStyle}>
      <div style={frameStyle} ref={frameRef}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative", overflow: "hidden" }}>
          ...
          {tab === "settings" && <SettingsScreen data={data} setData={setData} onBack={() => setTab("diario")} onFullOnboarding={() => { setStep(0); setOnboarded(false); }} onOpenTransactions={() => setTab("transactions")} onChangeUser={onChangeUser} onOpenRegime={() => setTab("regime")} onOpenImport={() => setTab("importcsv")} />}
