          ...
          {Number(data.mesiLavorati) > 0 && Number(data.mesiLavorati) < 12 && (data.stipendio || data.tariffaOrariaLorda) ? (
            <div style={{ display: "flex", alignItems: "center", gap: 8, backgroundColor: C.bg, border: `1px solid ${C.panelBorder}`, borderRadius: 6, padding: "8px 10px", marginBottom: 16 }}>
              ...
              <span style={{ fontSize: 11.5, color: C.textFaint, lineHeight: 1.4 }}>
                Per obiettivi e budget useremo circa <strong style={{ color: C.paper }}>{((data.usaOraria ? (Number(data.tariffaOrariaLorda) || 0) * ((Number(data.percentualeNetta) || 100) / 100) * (Number(data.oreSettimana) || 0) * 4.33 : Number(data.stipendio) || 0) * (Number(data.mesiLavorati) / 12)).toFixed(0)}€/mese</strong> in media sull'anno, non {data.usaOraria ? "quello di quando lavori" : Number(data.stipendio || 0).toFixed(0) + "€"}.
              </span>
            </div>
          ) : null}
        </>
