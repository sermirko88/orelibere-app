...
function WelcomeScreen({ onStart }) {
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "40px 24px 32px 24px", justifyContent: "space-between" }}>
      <div>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 800, color: C.paper, fontFamily: DISPLAY_FONT, margin: "0 0 20px 0", lineHeight: 1.15 }}>
          I soldi si dimenticano.<br />Le ore no.
        </h1>

        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", backgroundColor: C.panel, border: `1px solid ${C.panelBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <Calculator size={14} color={C.brass} />
            </div>
            <p style={{ fontSize: 13.5, color: C.textFaint, lineHeight: 1.55, margin: 0 }}>
              Ogni spesa che segni non la vedrai solo in euro, ma anche in <strong style={{ color: C.paper }}>ore del tuo lavoro</strong> — il modo più concreto per capire cosa ti costa davvero.
            </p>
          </div>
          <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", backgroundColor: C.panel, border: `1px solid ${C.panelBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <PiggyBank size={14} color={C.brass} />
            </div>
            <p style={{ fontSize: 13.5, color: C.textFaint, lineHeight: 1.55, margin: 0 }}>
              Puoi impostare <strong style={{ color: C.paper }}>obiettivi di risparmio</strong> e l'app ti dice se sono realistici rispetto a quello che guadagni e spendi davvero.
            </p>
          </div>
          <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
            <div style={{ width: 30, height: 30, borderRadius: "50%", backgroundColor: C.panel, border: `1px solid ${C.panelBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <HandCoins size={14} color={C.brass} />
            </div>
            <p style={{ fontSize: 13.5, color: C.textFaint, lineHeight: 1.55, margin: 0 }}>
              A fine settimana o fine mese, <strong style={{ color: C.paper }}>chiudi il periodo</strong>: decidi tu dove mandare quello che sei riuscito a risparmiare.
            </p>
          </div>
        </div>
      </div>
...
// Ogni passo si avanza con un tocco su "Avanti"; per saltarlo del tutto bisogna
// tenere premuto 2 secondi, così siamo certi che non sia un tocco distratto.
function TutorialOverlay({ onFinish }) {
  const [step, setStep] = useState(0);
  const steps = [
    { anchor: { top: 130 }, text: "Qui in alto vedi subito quante ore hai già \"speso\" oggi, divise tra spese fisse (viola) ed extra (arancio/rosso)." },
    { anchor: { bottom: 176 }, text: "Tocca il + per registrare una spesa: scegli la categoria, conferma l'importo, fatto in pochi secondi." },
    { anchor: { bottom: 96 }, text: "Da qui sotto passi tra Diario, Budget e Simulatore. Se colleghi un conto o hai risparmi da smistare, compaiono anche altre due voci." },
    { anchor: { top: 60, right: 20, left: "auto" }, text: "L'ingranaggio in alto apre le Impostazioni: da lì colleghi i conti, cambi il periodo di chiusura o rivedi i dati iniziali." },
  ];
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 100, backgroundColor: "rgba(20,15,10,0.72)" }}>
      <div style={{
        position: "absolute", left: 20, right: 20, width: current.anchor.right ? 220 : undefined,
        ...current.anchor,
        ...
        <button onClick={next} style={{ width: "100%", padding: "10px 0", borderRadius: 6, border: "none", backgroundColor: C.brass, color: C.ink, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
...
function MainApp({ currentUser, onChangeUser }) {
  ...
  return (
    <div style={outerStyle}>
      <div style={frameStyle}>
        {!tutorialDone && <TutorialOverlay onFinish={() => setTutorialDone(true)} />}
        {syncError && (
          <div style={{ position: "absolute", top: 8, left: 8, right: 8, zIndex: 90, backgroundColor: C.rust, color: "#fff", borderRadius: 8, padding: "10px 12px", fontSize: 11.5, lineHeight: 1.4, display: "flex", alignItems: "flex-start", gap: 8 }}>
            <span style={{ flex: 1 }}>⚠ Sincronizzazione: {syncError}</span>
            <button onClick={() => setSyncError(null)} style={{ background: "none", border: "none", color: "#fff", cursor: "pointer", fontWeight: 700, padding: 0 }}>✕</button>
